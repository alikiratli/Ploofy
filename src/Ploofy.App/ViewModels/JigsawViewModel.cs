...
/// </summary>
/// <remarks>
/// Kurallar <see cref="JigsawRound"/>, kesim ve çizim <c>JigsawSurface</c>
/// içinde. Burada kalan iş üstteki bilgi şeridi ve turun bitişini ilerleme
/// kaydına bağlamak — Şekil Ayırma ile aynı kalıp.
/// </remarks>
public sealed partial class JigsawViewModel : ObservableObject, IDisposable
{
    ...
    [ObservableProperty]
    public partial int Placed { get; set; }
    ...
    public event EventHandler<JigsawRound>? RoundReady;

    public async Task LoadAsync()
    {
        var session = _flow.PendingSession;
        if (session is null || session.GameId != GameCatalog.Jigsaw)
        {
    ...
    public void OnDropped(PlaceOutcome outcome)
    {
        switch (outcome)
        {
    ...
    private void SyncPips()
    {
        ...
        Placed = _round.Placed;
        for (var i = 0; i < Pips.Count; i++)
        {
            Pips[i].IsFilled = i < _round.Placed;
        }
    }
    ...
    public async Task CompleteRoundAsync()
    {
        ...
        var outcome = new RoundOutcome(
            ...
            player.ProfileId,
            player.Band,
            // Bu oyunda kaybetme yok: tur ancak bütün parçalar yerleşince
            // bitiyor, yanlış yuva yalnızca yıldızı etkiliyor.
            Completed: true,
            Correct: round.Placed,
