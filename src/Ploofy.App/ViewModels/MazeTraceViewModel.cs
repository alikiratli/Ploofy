...
/// </summary>
/// <remarks>
/// Kurallar <see cref="MazeTraceRound"/>, çizim ve parmak takibi
/// <c>MazeTraceSurface</c> içinde. Burada kalan iş üstteki bilgi şeridi ve
/// turun bitişini ilerleme kaydına bağlamak.
/// </remarks>
public sealed partial class MazeTraceViewModel : ObservableObject, IDisposable
{
    ...
    private MazeTraceRound? _round;
    private readonly Stopwatch _clock = new();
    private readonly List<PlayerResult> _results = [];

    public MazeTraceViewModel(
    ...
    public event EventHandler<MazeTraceRound>? RoundReady;

    public async Task LoadAsync()
    {
        var session = _flow.PendingSession;
        if (session is null || session.GameId != GameCatalog.MazeTrace)
        {
    ...
    public void OnTraced(TraceOutcome outcome)
    {
        ...
        switch (outcome)
        {
            ...
            case TraceOutcome.Slipped:
                ...
                // kez bildiriyor, yoldan uzakta gezinen parmak sesi
                // tekrarlamıyor.
    ...
    public async Task CompleteRoundAsync()
    {
        ...
        var outcome = new RoundOutcome(
            ...
            player.ProfileId,
            player.Band,
            // Bu oyunda kaybetme yok: yoldan çıkmak turu bitirmiyor, yalnızca
            // yıldızı etkiliyor.
            Completed: true,
            Correct: round.Completed,
            Mistakes: round.Mistakes,
            Elapsed: _clock.Elapsed,
            // Hedef süre yok: yolu hızlı çizmek daha iyi çizmek değil, tersine
            // köşeyi kesip geçmeyi ödüllendirirdi. Bkz. MazeTraceTuning.
            ParTime: null);
