...
/// <summary>
/// Şekil Ayırma'nın ekranı.
/// </summary>
/// <remarks>
/// Kurallar <see cref="ShapeSortRound"/>, çizim ve sürükleme
/// <c>ShapeSortSurface</c> içinde. Burada kalan iş üstteki bilgi şeridi ve
/// turun bitişini ilerleme kaydına bağlamak — Balon Patlatma ile aynı kalıp.
/// </remarks>
public sealed partial class ShapeSortViewModel : ObservableObject, IDisposable
{
    ...
    private ShapeSortRound? _round;
    private readonly Stopwatch _clock = new();
    private readonly List<PlayerResult> _results = [];

    public ShapeSortViewModel(
    ...
    public event EventHandler<ShapeSortRound>? RoundReady;

    public async Task LoadAsync()
    {
        var session = _flow.PendingSession;
        if (session is null || session.GameId != GameCatalog.ShapeSort)
        {
    ...
    public void OnDropped(DropOutcome outcome)
    {
        switch (outcome)
        {
    ...
    private void SyncPips()
    {
        ...
        Sorted = _round.Sorted;
        for (var i = 0; i < Pips.Count; i++)
        {
            Pips[i].IsFilled = i < _round.Sorted;
        }
    }
    ...
    public async Task CompleteRoundAsync()
    {
        ...
        var outcome = new RoundOutcome(
            ...
            player.ProfileId,
            player.Band,
            // Bu oyunda kaybetme yok: tur ancak bütün parçalar ayrılınca bitiyor.
            Completed: true,
